...
@pytest.mark.asyncio
async def test_mergeable_pull_request_merged_delete_branch_dependent_prs(
    api: MockPrApi,
    config: V1,
    config_path: str,
    config_str: str,
    ...
    Here we test with the delete_branch_on_merge config enabled but there are associate PRs. When we have other dependent PRs on a branch, we cannot delete it.
    """
    pull_request.state = PullRequestState.MERGED
    config.merge.delete_branch_on_merge = True
    ...
    await mergeable(
        ...
        api_call_retry_method_name=None,
        #
        associated_pr_numbers={pull_request.number, 1554},
    )
    assert api.set_status.call_count == 0
    assert api.dequeue.call_count == 1
    assert api.delete_branch.call_count == 0
